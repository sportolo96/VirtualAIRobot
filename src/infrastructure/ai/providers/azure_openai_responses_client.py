...
class AzureOpenAIResponsesClient:
    ...
    def _complete(
        ...
        if not text:
            raise RuntimeError("Azure OpenAI response did not contain text output")
        return text
    ...
    def _post_json(self, path: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Azure OpenAI request failed with HTTP {exc.code}: {detail}"
            ) from exc
        ...
        if not isinstance(parsed, dict):
            raise RuntimeError("Azure OpenAI response payload was not an object")
        return parsed

    def _build_url(self, path: str) -> str:
        base = f"{self._api_base_url}{path}"
